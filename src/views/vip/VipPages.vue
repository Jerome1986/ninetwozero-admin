...
const userListGet = async (token: string, pageNum: number = 1, pageSize: number = 10) => {
  loading.value = true
  const res = await userGetApi(token, pageNum, pageSize, 'vip')
  console.log('返回', res)
...
const searchMaterialGet = async () => {
  const res = await searchUserApi(userStore.token, searchValue.value, 'vip')
...
<template>
  ...
  <PageContainer title="用户列表">
    ...
    <el-table :data="userList" style="width: 100%" v-loading="loading">
      ...
      <el-table-column label="会员到期时间" align="center" prop="registerTime" width="160">
        <template #default="{ row }">
